...
namespace PixelPhoto.Activities.Store
{
    ...
    public class EditStoreActivity : BaseActivity, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback
    {
        ...
        private string StoreId ,PathImage, TypeDialog, CategoryId, LicenseTypeId;
        private StoreDataObject DataStore;
        ...
        private void InitToolbar()
        {
            try
            {
                ...
                if (toolbar != null)
                {
                    toolbar.Title = GetString(Resource.String.Lbl_EditStore);
        ...
        private async void TxtAddOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                var dict = new Dictionary<string, string>
                {
                    ...
                    {"tags", TxtTags.Text},
                    {"category", CategoryId},
                    //{"license[" + LicenseTypeId + "]", TxtPrice.Text}
                };

                var (apiStatus, respond) = await RequestsAsync.Store.EditStoreImage(StoreId ,dict, LicenseTypeId , TxtPrice.Text, PathImage);
                if (apiStatus == 200)
                {
                    if (respond is MessageObject result)
                    {
                        ...
                        Toast.MakeText(this, GetText(Resource.String.Lbl_YourImageWasSuccessfullyUpdated), ToastLength.Short)?.Show();
                        AndHUD.Shared.Dismiss(this);

                        var data = new StoreDataObject
                        {
                            Tags = TxtTags.Text,
                            Title = TxtTitle.Text,
        ...
        private void LoadData()
        {
            try
            { 
                DataStore = JsonConvert.DeserializeObject<StoreDataObject>(Intent?.GetStringExtra("StoreItem"));
                if (DataStore != null)
                {
                    TxtTitle.Text = Methods.FunString.DecodeString(DataStore.Title);
                    TxtCategory.Text = Methods.FunString.DecodeString(DataStore.CategoryName);

                    if (DataStore.LicenseOptions?.LicenseOptionsClass != null)
                    { 
                        if (DataStore.LicenseOptions?.LicenseOptionsClass.RightsManagedLicense > 0)
                        {
                            TxtLicenseType.Text = GetText(Resource.String.Lbl_rights_managed_license);
                            TxtPrice.Text = DataStore.LicenseOptions?.LicenseOptionsClass.RightsManagedLicense.Value.ToString(); 
                        }
                        else if (DataStore.LicenseOptions?.LicenseOptionsClass.EditorialUseLicense > 0)
                        {
                            TxtLicenseType.Text = GetText(Resource.String.Lbl_editorial_use_license);
                            TxtPrice.Text = DataStore.LicenseOptions?.LicenseOptionsClass.EditorialUseLicense.Value.ToString();
                        }
                        else if (DataStore.LicenseOptions?.LicenseOptionsClass.RoyaltyFreeLicense > 0)
                        {
                            TxtLicenseType.Text = GetText(Resource.String.Lbl_royalty_free_license);
                            TxtPrice.Text = DataStore.LicenseOptions?.LicenseOptionsClass.RoyaltyFreeLicense.Value.ToString();
                        } 
                        else if (DataStore.LicenseOptions?.LicenseOptionsClass.RoyaltyFreeExtendedLicense > 0)
                        {
                            TxtLicenseType.Text = GetText(Resource.String.Lbl_royalty_free_extended_license);
                            TxtPrice.Text = DataStore.LicenseOptions?.LicenseOptionsClass.RoyaltyFreeExtendedLicense.Value.ToString();
                        }
                        else if (DataStore.LicenseOptions?.LicenseOptionsClass.CreativeCommonsLicense > 0)
                        {
                            TxtLicenseType.Text = GetText(Resource.String.Lbl_creative_commons_license);
                            TxtPrice.Text = DataStore.LicenseOptions?.LicenseOptionsClass.CreativeCommonsLicense.Value.ToString();
                        }
                        else if (DataStore.LicenseOptions?.LicenseOptionsClass.PublicDomain > 0)
                        {
                            TxtLicenseType.Text = GetText(Resource.String.Lbl_public_domain);
                            TxtPrice.Text = DataStore.LicenseOptions?.LicenseOptionsClass.PublicDomain.Value.ToString();
                        }
                        else if (DataStore.LicenseOptions?.LicenseOptionsClass.Empty > 0)
                        {
                            TxtLicenseType.Text = "";
                            TxtPrice.Text = DataStore.LicenseOptions?.LicenseOptionsClass.Empty.Value.ToString();
                        }
                        else
                        {
                            TxtLicenseType.Text = "";
