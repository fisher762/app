    ...
    public class TemplateView : FrameLayout
    {
        ...
        private void ApplyStyles()
        {
            try
            {
                ...
                var primary = Styles.GetPrimaryTextTypeface();
                if (primary != null)
                {
                    PrimaryView?.SetTypeface(primary, TypefaceStyle.Normal);
                }

                var secondary = Styles.GetSecondaryTextTypeface();
                if (secondary != null)
                {
                    SecondaryView?.SetTypeface(secondary, TypefaceStyle.Normal);
                }

                var tertiary = Styles.GetTertiaryTextTypeface();
                if (tertiary != null)
                {
                ...
                var primaryTypefaceColor = Styles.GetPrimaryTextTypefaceColor();
                if (primaryTypefaceColor > 0)
                {
                    PrimaryView?.SetTextColor(primaryTypefaceColor);
                }

                var secondaryTypefaceColor = Styles.GetSecondaryTextTypefaceColor();
                if (secondaryTypefaceColor > 0)
                {
                    SecondaryView?.SetTextColor(secondaryTypefaceColor);
                }

                var tertiaryTypefaceColor = Styles.GetTertiaryTextTypefaceColor();
        ...
        private void InitView(Context context, IAttributeSet attributeSet)
        {
            try
            {
                var attributes = context.Theme.ObtainStyledAttributes(attributeSet, Resource.Styleable.TemplateView, 0, 0);
        ...
        public void NativeContentAdView(UnifiedNativeAd nativeAd)
        {
            try
            {
                ...
                // have a video asset.
                var vc = nativeAd.VideoController;
