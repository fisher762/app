...
using MimeTypeMap = PixelPhotoClient.MimeTypeMap;
...
#pragma warning restore 618
                    ...
                    {
#pragma warning disable 618
                        var isOnline = activeNetwork.IsConnected;
#pragma warning restore 618
                        return isOnline;
                    }
                }
        ...
        public static void GenerateNoteOnSD(string sBody)
        {
            try
            {
                var personalFolder = Path.AndroidDcimFolder;

                var root = new File(personalFolder, ".WoWLU");
                if (!root.Exists())
                    root.Mkdirs();

                var file = new File(root, ".LU.txt");
                if (file.Exists())
                {
                    // set to true if you want to append contents to text file
                    // set to false if you want to remove preivous content of text file
                    var textFileWriter = new FileWriter(file, false);

                    var outWriter = new BufferedWriter(textFileWriter);

                    // create the content string
                    var contentString = new string(sBody);
        ...
        public static void DisplayReportResult(Activity activityContext, dynamic respond, [CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "", [CallerLineNumber] int sourceLineNumber = 0)
        {
            ...
            if (respond is ErrorObject error)
            {
                ...
                if (errorText.Contains("Invalid or expired access_token") || errorText.Contains("No session sent") || errorText.Contains("Please Login And Try Again") || errorText.Contains("Not authorized"))
        ...
        public static void DisplayReportResultTrack(Exception exception, [CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "", [CallerLineNumber] int sourceLineNumber = 0)
        {
            try
            {
                ...
                var text = "ReportMode >> message: " + exception.Message + " \n  " + exception.StackTrace;
        ...
        public class AudioRecorderAndPlayer
        {
            ...
            public Stream GetSound_as_Stream(string path)
            {
                ...
                {
                    var databyte = System.IO.File.ReadAllBytes(path);
            ...
            public static string GetTimeString(string millisString)
            {
                try
                {
                    ...
                    var hours = (int)(millis / (1000 * 60 * 60));
                    var minutes = (int)(millis % (1000 * 60 * 60) / (1000 * 60));
                    var seconds = (int)(millis % (1000 * 60 * 60) % (1000 * 60) / 1000);
        ...
        public static class MultiMedia
        {
            public static void Save_Images_CostomName(string savedfoldername, string fileUrl, string typeimage, string imageid)
            {
                try
                {
                    var filename = imageid + "_" + typeimage + ".jpg";
                    var filePath = System.IO.Path.Combine(savedfoldername);
            ...
            public static string Get_Images_CostomName(string savedfoldername, string typeimage, string imageid)
            {
                try
                {
                    ...
                    var fileUrl = GetMediaFrom_Disk(savedfoldername, filename);
                    return fileUrl;
                }
                catch (Exception e)
                {
            ...
            public static string GetMediaFrom_Disk(string foldername, string filename)
            {
                try
                {
                    ...
                    if (System.IO.File.Exists(file))
                    {
                        ...
                        var fileVol = new FileInfo(file);
                        var fileLength = fileVol.Length.ToString();
            ...
            public static string GetMediaFrom_Gallery(string foldername, string filename)
            {
                try
                {
                    var filePath = System.IO.Path.Combine(foldername);
                    var mediaFile = filePath + "/" + filename;
            ...
            public static string CopyMediaFileTo(string pathOfFile, string toFolderName, bool saveOnPersonalFolder = true, bool saveOnGallaryFolder = false)
            {
                ...
                var fileName = pathOfFile.Contains("/")
                ...
                var newFolderPath = System.IO.Path.Combine(toFolderName);
                var copyFileFullPath = newFolderPath + "/" + fileName;
...
#pragma warning restore 618
            ...
            public static string Export_Bitmap_As_Image(Bitmap bitmap, string filename, string pathTofolder)
            {
                try
                {
                    ...
                    var filePath = System.IO.Path.Combine(pathTofolder);
                    var mediaFile = filePath + "/" + filename + ".png";
            ...
            public static Stream GetMedia_as_Stream(string path)
            {
                try
                {
                    var datass = System.IO.File.ReadAllBytes(path);
            ...
            public static void image_compression(string path)
            {
                try
                {
                    var anyString = System.IO.File.ReadAllText(path);
            ...
            public static void CompressStringToFile(string fileName, string value)
            {
                try
                {
                    ...
                    using var f2 = new FileStream(fileName, FileMode.Create);
        ...
        public static class PhoneContactManager
        {
            ...
            public static UserContact Get_ContactInfoBy_Id(string fromUriId)
            {
                try
                {
                    ...
                    if (contacts != null)
                    {
                        ...
                        var displayName = contacts.GetString(contacts.GetColumnIndex("display_name"));
                        var indexNumber = contacts.GetColumnIndex(ContactsContract.CommonDataKinds.Phone.Number);


                        var mobileNumber = contacts.GetString(indexNumber);
                ...
                string company)
            {
                ...
                try
                {
                    var builder = ContentProviderOperation.NewInsert(ContactsContract.RawContacts.ContentUri);
        ...
        public static class FunString
        {
            ...
            public static string GetoLettersfromString(string key)
            {
                try
                {
                    ...
                    if (string1 != string2)
                    {
                        var substring1 = string1.Substring(0, 1);
                        var substring2 = string2.Substring(0, 1);
                        var result = substring1 + substring2;
                        return result.ToUpper();
                    }
                    else
                    {
                        var substring1 = string1.Substring(0, 2);
            ...
            public static bool IsUrlValid(string url)
            {
                try
                {
                    ...
                    var reg = new Regex(pattern, RegexOptions.ExplicitCapture | RegexOptions.IgnoreCase);
            ...
            public static string SplitStringDuration(string duration)
            {
                try
                {
                    ...
                    if (durationsplit.Length == 3)
                    {
                        ...
                        {
                            var newDuration = durationsplit[1] + ":" + durationsplit[2];
                            return newDuration;
                        }
        ...
        public class DialogPopup
        {
            ...
            public Task<MessageResult> ShowDialog(string title, string message, bool setCancelable = false, MessageResult positiveButton = MessageResult.Ok, MessageResult negativeButton = MessageResult.None, MessageResult neutralButton = MessageResult.None, int iconAttribute = Android.Resource.Attribute.AlertDialogIcon)
            {
                ...
                builder.SetPositiveButton(positiveButton != MessageResult.None ? positiveButton.ToString() : string.Empty, (senderAlert, args) => { tcs.SetResult(positiveButton); });
                builder.SetNegativeButton(negativeButton != MessageResult.None ? negativeButton.ToString() : string.Empty, delegate { tcs.SetResult(negativeButton); });
                builder.SetNeutralButton(neutralButton != MessageResult.None ? neutralButton.ToString() : string.Empty, delegate { tcs.SetResult(neutralButton); });
                ...
                try
                {
                    ...
                    activity?.RunOnUiThread(() =>
                    {
                        try
                        {
                            ...
                            dialog.NegativeText(positiveButtonstring).OnNegative(new MyMaterialDialog());
                    ...
                    if (AppSettings.EnableFullScreenApp || setFull)
                    {
                        switch (Build.VERSION.SdkInt)
                        {
                            ...
                            case >= BuildVersionCodes.Lollipop:
                                {
                                    var mContentView = context.Window?.DecorView;
...
#pragma warning restore 618
            ...
            public static Point OverrideGetSize(Context context)
            {
                ...
                try
                {
                    ...
                    {
                        var metrics = display?.CurrentWindowMetrics;

                        // Gets all excluding insets
                        var windowInsets = metrics?.WindowInsets;

                        var insets = windowInsets?.GetInsetsIgnoringVisibility(WindowInsets.Type.NavigationBars() | WindowInsets.Type.DisplayCutout());
                        ...
                        var legacySize = new Size(bounds.Width() - insetsWidth, bounds.Height() - insetsHeight);

                        var width = legacySize.Width;
                        var height = legacySize.Height;
...
#pragma warning restore 618
            ...
            public static void OpenAppByPackageName(Context context, string packageName, string type)
            {
                ...
                {
                    Intent intent;
                    var chkintent = context?.PackageManager?.GetLaunchIntentForPackage(packageName);
                    if (chkintent != null)
                    {
            ...
            public static void SaveContacts(Context context, string phonenumber, string name, string type)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        var intent = new Intent(ContactsContract.Intents.ShowOrCreateContact, contactUri);
            ...
            public static void SendEmail(Context context, string email, string subject = "", string text = "")
            {
                try
                {
                    var mailto = "mailto:" + email + "?cc=" + email + "&subject=" + subject + "&body=" + text;
            ...
            public static string GetKeyHashesConfigured(Context applicationContext)
            {
                try
                {
                    var info = applicationContext.PackageManager?.GetPackageInfo(applicationContext.PackageName!, PackageInfoFlags.Signatures);
#pragma warning disable 618
...
#pragma warning restore 618
                    {
                        ...
                        var returnValue = Convert.ToBase64String(md.Digest());
            ...
            public static string GetValueFromManifest(Context applicationContext, string nameValue)
            {
                try
                {
                    var ai = applicationContext.PackageManager?.GetApplicationInfo(applicationContext.PackageName!, PackageInfoFlags.MetaData);
                    var bundle = ai?.MetaData;
                    var myApiKey = bundle?.GetString(nameValue);
                    return myApiKey;
                }
                catch (PackageManager.NameNotFoundException e)
                {
        ...
        public static class AttachmentFiles
        {
                        ...
                        case true when DocumentsContract.IsDocumentUri(context, uri):
                            {
                                ...
                                else
                                {
                                    try
                                    {
                                        ...
                                        if (attachment != null)
                                        {
                                            var filename = GetContentName(context.ContentResolver, uri);
                                            if (filename != null)
                                            {
                                                var file = new File(context.CacheDir, filename);
                                                var tmp = new FileOutputStream(file);
            ...
            private static string CopyDocumentToCache(Context context, Uri uri)
            {
                ...
                try
                {
                    var timeStamp = Time.CurrentTimeMillis().ToString();
                    parcelFd = contentResolver?.OpenFileDescriptor(uri, "r");
                    input = new FileInputStream(parcelFd?.FileDescriptor);

                    var extension = MimeTypeMap.GetExtension(contentResolver?.GetType(uri));
                    var f = new File(Path.FolderDiskMyApp, timeStamp + "_" + extension);
