...
namespace PixelPhoto.Helpers.Utils
{
    public static class ShareFileImplementation
    {
        ...
        /// <param name="postLink">name of the file</param>
        /// <param name="title">Title of popup on share (not included in message)</param>
        /// <returns>awaitable bool</returns>
        public static async Task ShareRemoteFile(string fileUri, string fileName, string postLink, string title)
        {
            try
            {
                Download(fileUri, fileName, postLink, title);
                await Task.Delay(0);
            }
            catch (Exception ex)
            {
        ...
        public static void Download(string imageUrl, string fileName, string postLink, string title)
        {
            try
            {
                if (string.IsNullOrEmpty(imageUrl) || string.IsNullOrEmpty(postLink) || Activity == null)
                    return;
                            ...
                            if (!File.Exists(mediaFile))
                            {
                                ...
                                webClient.DownloadDataCompleted += (s, e) =>
                                {
                                    try
                                    {
                                        ...
                                        {
                                            var file2 = new Java.IO.File(getImagePath);
        ...
        //    {
        //        ////Console.WriteLine("Unity", "Original media file is missing or inaccessible!");
        //        return;
        //    }
        ...
        //            catch (Exception e)
        //            {
        //                //Console.WriteLine("Unity", "Exception:", e);
