    ...
    public class StReadMoreOption
    {
        ...
        public void AddReadLess(TextView textView, ICharSequence text)
        {
            try
            {
                ...
                var spendableStringBuilder = new SpannableStringBuilder(text);
                spendableStringBuilder.Append("\n ");
                spendableStringBuilder.Append(LessLabel);

                var ss = SpannableString.ValueOf(spendableStringBuilder);
        ...
        private class StRunnable : Object, IRunnable
        {
            ...
            public void Run()
            {
                try
                {
                    var textLengthNew = Option.TextLength;
                    ...
                    { 
                        ...
                        var lp = (ViewGroup.MarginLayoutParams)TextView.LayoutParameters;

                        var subString = Text.ToString().Substring(TextView.Layout.GetLineStart(0), TextView.Layout.GetLineEnd(Option.TextLength - 1));
                        textLengthNew = subString.Length - (Option.MoreLabel.Length + 4 + lp.RightMargin / 6);
                    }

                    var spendableStringBuilder = new SpannableStringBuilder(Text.SubSequence(0, textLengthNew));
                    spendableStringBuilder.Append(" ...");
