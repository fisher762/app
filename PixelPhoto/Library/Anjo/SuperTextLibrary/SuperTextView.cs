    ...
    public class SuperTextView : TextView
    {
        ...
        public override void SetText(ICharSequence text, BufferType type)
        {
            try
            {
                ...
                var spannableString = MakeSpannAbleString(text);
        ...
        private SpannableString MakeSpannAbleString(ICharSequence text)
        {
            try
            {
                ...
                var autoLinkItems = MatchedRanges(text);

                foreach (var autoLinkItem in autoLinkItems)
                {
                    Console.WriteLine("Run foreach autoLinkItem => 135");
                    var currentColor = GetColorByMode(autoLinkItem.GetAutoLinkMode());
                    var clickAbleSpan = new XTouchableSpan(this, autoLinkItem, currentColor, DefaultSelectedColor, IsUnderLineEnabled);
            ...
            {
                Methods.DisplayReportResultTrack(e);
                var spendableString = new SpannableString(text);
        ...
        private List<StTools.XAutoLinkItem> MatchedRanges(ICharSequence text)
        {
            try
            {
                var autoLinkItems = new List<StTools.XAutoLinkItem>();
                ...
                foreach (var anAutoLinkMode in AutoLinkModes)
                {
                    Console.WriteLine("Run foreach MatchedRanges => 175");
                    var regex = StTools.XUtils.GetRegexByAutoLinkMode(anAutoLinkMode, CustomRegex);
                    ...
                    { 
                        while (matcher.Find())
                        {
                            ...
                            var ss = new StTools.XAutoLinkItem(matcher.Start(), matcher.End(), matcher.Group(), anAutoLinkMode, UserId);
        ...
        public void SetBoldAutoLinkModes(List<StTools.XAutoLinkMode> autoLinkModes)
        {
            try
            {
                ...
                foreach (var autoLinkMode in autoLinkModes)
                {
        ...
        protected override void OnMeasure(int widthMeasureSpec, int heightMeasureSpec)
        {
            try
            {
                if (Build.VERSION.SdkInt >= (BuildVersionCodes)16)
                {
                    ...
                    var klass = Class.FromType(typeof(DynamicLayout));

                    try
                    {
                        var insetsDirtyField = klass.GetDeclaredField("sStaticLayout");
