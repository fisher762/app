    ...
    public class MultiTouchListener : Java.Lang.Object, View.IOnTouchListener
    {
        ...
        public static void Move(View view, TransformInfo info)
        {
            try
            {
                ...
                var rotation = AdjustAngle(view.Rotation + info.DeltaAngle);
        ...
        public static void ComputeRenderOffset(View view, float pivotX, float pivotY)
        {
            try
            {
                ...
                var offsetX = currPoint[0] - prevPoint[0];
                var offsetY = currPoint[1] - prevPoint[1];
        ...
        public bool OnTouch(View v, MotionEvent @event)
        {
            try
            {
                ...
                var x = (int)@event.RawX;
                var y = (int)@event.RawY;
                ...
                switch (action & @event.ActionMasked)
                {
                    ...
                    case MotionEventActions.Move:
                        var pointerIndexMove = @event.FindPointerIndex(MActivePointerId);
                        if (pointerIndexMove != -1)
                        {
                            var currX = @event.GetX(pointerIndexMove);
                            var currY = @event.GetY(pointerIndexMove);
                    ...
                    case MotionEventActions.PointerUp:
                        ...
                        var pointerIndex = (int)(@event.Action & MotionEventActions.PointerIndexMask) >> (int)MotionEventActions.PointerIndexShift;
                        var pointerId = @event.GetPointerId(pointerIndex);
                        if (pointerId == MActivePointerId)
                        {
                            var newPointerIndex = pointerId == 0 ? 1 : 0;
