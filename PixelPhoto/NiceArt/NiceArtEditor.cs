    ...
    public class NiceArtEditor : Java.Lang.Object, INiceArt.IBrushViewChangeListener, INiceArt.IOnGestureControl, INiceArt.IOnSaveBitmap, INiceArt.IOnMultiTouchListener
    {
        ...
        public View GetLayout(ViewType viewType)
        {
            try
            {
                ...
                if (viewType == ViewType.Text)
                {
                    ...
                    var txtText = rootView.FindViewById<TextView>(Resource.Id.tvNiceArtText);
                ...
                {
                    rootView = MLayoutInflater.Inflate(Resource.Layout.Style_NiceArt_TextView, null);
                    var txtTextEmojis = rootView.FindViewById<TextView>(Resource.Id.tvNiceArtText);
        ...
        public void AddImage(Bitmap desiredImage)
        {
            try
            {
                ...
                var imgCloseImage = ImageRootView.FindViewById<TextView>(Resource.Id.txtNiceArtClose);
                ...
                var multiTouchListener = GetMultiTouchListener();
        ...
        public void OnClick()
        {
            try
            {
                var isBackgroundVisible = FrmBorder.Tag != null && (bool)FrmBorder.Tag;
        ...
        public void AddText(Typeface textTypeface, string text, string colorCodeTextView)
        {
            try
            {
                ...
                var imgCloseText = TextRootView.FindViewById<TextView>(Resource.Id.txtNiceArtClose);
        ...
        public void ImgEditOnClick(object sender, EventArgs e)
        {
            try
            {
                var textInput = TextInputTv.Text;
                var currentTextColor = TextInputTv.CurrentTextColor;
        ...
        public void AddEmojis(Typeface emojisTypeface, string emojisName)
        {
            try
            {
                ...
                var imgCloseEmojis = EmojiRootView.FindViewById<TextView>(Resource.Id.txtNiceArtClose);
        ...
        public void AddViewToParent(View rootView, ViewType viewType)
        {
            try
            {
                var Params = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WrapContent, ViewGroup.LayoutParams.WrapContent);
        ...
        public MultiTouchListener GetMultiTouchListener()
        {
            try
            {
                var multiTouchListener = new MultiTouchListener(
                    DeleteView,
                    ParentView,
                    ImageView,
                    IsTextPinchZoomable,
        ...
        public void ClearHelperBox()
        {
            try
            {
                for (var i = 0; i < ParentView.ChildCount; i++)
                {
                    var childAt = ParentView.GetChildAt(i);
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        { 
                            ...
                            {
                                var drawingCache = BitmapUtil.LoadBitmapFromView(ParentView);
        ...
        public void OnBitmapReady(Bitmap saveBitmap, SaveType type)
        {
            try
            {
                var task = new MyAsyncTask(this, ImagePath, ParentView, OnSaveListener, SaveSettings, saveBitmap, type);
                task.Execute();
            }
            catch (Exception e)
            {
        ...
        public static string ConvertEmoji(string emoji)
        {
            try
            {
                ...
                try
                {
                    var convertEmojiToInt = Convert.ToInt32(emoji.Substring(2), 16);
        ...
        public static List<string> GetEmojis(Context context)
        {
            try
            {
                var emojiList = context.Resources?.GetStringArray(Resource.Array.NiceArtEmojis);
