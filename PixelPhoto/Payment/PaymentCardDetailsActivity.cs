...
namespace PixelPhoto.Payment
{
    ...
    public class PaymentCardDetailsActivity : BaseActivity, ITokenCallback, PaymentSession.IPaymentSessionListener, IEphemeralKeyProvider, IPaymentCompletionProvider, ISourceCallback
    {
        ...
        private string Price, PayType, Id, tokenId, LicenseType;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            try
            {
                ...
                LicenseType = Intent?.GetStringExtra("LicenseType") ?? "";
        ...
        private void MultilineWidgetOnCvcComplete(object sender, EventArgs e)
        {
            try
            {
                if (MultilineWidget.Card != null && MultilineWidget.Card.ValidateCard() && MultilineWidget.ValidateAllFields())
                {
                    ...
                    {
                        var number = InsertPeriodically(MultilineWidget.Card.Number.Trim(), " ", 4);
        ...
        private string InsertPeriodically(string text, string insert, int period)
        {
            try
            {
                ...
                var formatted = string.Join(insert, parts);
                return formatted;
            }
            catch (Exception e)
            {
        ...
        public async void OnSuccess(Source p0)
        {
            try
            {
                ...
                if (p0.Status.Contains("succeeded"))
                {
                    ...
                    if (PayType == "Funding")
                    {
                        (var apiStatus, var respond) = await RequestsAsync.Funding.FundingPay(Id, Price);
                        if (apiStatus == 200)
                        {
                    ...
                    if (PayType == "Store")
                    {
                        (var apiStatus, var respond) = await RequestsAsync.Store.StoreBuy(Id, LicenseType, Price);
                        if (apiStatus == 200)
                        {
                            StoreViewActivity.GetInstance()?.StartApiService();
                    ...
                    //    {
                    //        (int apiStatus, var respond) = await RequestsAsync.Global.SetProAsync(Id);
                    //        if (apiStatus == 200)
                    //        {
        ...
        public void OnSuccess(Token token)
        {
            try
            {
                ...
                var currencyCode = /*ListUtils.SettingsSiteList?.StripeCurrency ??*/ AppSettings.CurrencyCodeStatic;
        ...
        private PaymentSessionConfig GetPaymentSessionConfig()
        {
            try
            {
